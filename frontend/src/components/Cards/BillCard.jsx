...
const BillCard = ({ billflag,setbillflag,user,BillData,thememode }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const editBill = async () => {
      try {
        const res = await axios.put(`https://equipay-gqsl.onrender.com/${BillData._id}`, { BillInput });
  ...
  const handleDelete = (id) => {
    const delBill = async (id) => {
      try {
        const res = await axios.delete(`https://equipay-gqsl.onrender.com/api/bills/deleteBill/${id}`);
        console.log(res.data);
        setbillflag((prev)=>!(prev))
      } catch (err) {
        console.log(err);
