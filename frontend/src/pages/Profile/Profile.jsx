...
function Profile({ user, thememode, toggle,setUser}) {
      ...
      .then(() => {
        getDownloadURL(imageRef)
          .then((url) => {
            const addUrl= async()=>{
              try{
                const res = await axios.put(`https://equipay-gqsl.onrender.com/api/user/addImg/${user._id}`,{url})
  ...
  React.useEffect(() => {
    const getBadges = async () => {
      try {
        const response = await axios.get(`https://equipay-gqsl.onrender.com/api/user/getBadges/${user._id}`);
